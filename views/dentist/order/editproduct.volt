...
{% block content %}
    ...
    <form id="orderForm" action="{{ url('dentist/order/editproduct/' ~ recipe.id ) }}"
            ...
            <div class="col-md-8">
                <div class="form-group">
                    <h3>
                        ...
                        {% endif %}
                        {#{{ recipe.custom_name }}#} <span class="pull-right recipeprice">{% if recipe.Recipes.price_type is 2 %}{{ recipe.Recipes.price }}{% endif %}</span>
                    </h3>
                    <hr/>
                    <p>
                    ...
                    {% for customField in recipe.DentistOrderRecipeData %}
                        ...
                        <div class="form-group">
                            ...
                            {% if customField.field_type === "text" %}
                                {{ text_field('customField['~customField.id~']', 'class': 'form-control', 'value': customField.field_value) }}
                                {% elseif customField.field_type === "number" %}
                                {{ numeric_field('customField['~customField.id~']', 'class': 'form-control', 'value': customField.field_value) }}
                            ...
                            {% elseif customField.field_type === "textarea" %}
                                <textarea name="customField[{{ customField.id }}]" class="form-control tinymce-limited" style="height: 150px;">{{ customField.field_value }}</textarea>
                            {% endif %}
                        </div>
                        {% endif %}
                    {% endfor %}
                ...
                {#<div class="form-group">#}
                    {#{{ text_area('description', 'placeholder': 'Order notes or remarks...'|t, 'class': 'form-control', 'value': recipe.DentistOrder.description) }}#}
                {#</div>#}
            </div>
        </div>
    ...
    <script>
        $(function () {
            ...
            function recalculatePrice() {
                ...
                $.post("{{ url('dentist/order/calculateprice/' ~ recipe.Recipes.code) }}", {
            ...
            $(document).on('click', '.remove-file', function(){
                $('#rowFile_'+$(this).attr('data-id')).remove();
                $('#deletedFile_'+$(this).attr('data-id')).val(1);
            });
            ...
            $('#confirmButton').on('click', function(){
                ...
                var content = '<tr data-phase="'+phase+'" data-days="'+days+'" id="row_'+counter+'" class="delivery-row">' +
                    ...
                    '<input id="dateval_'+counter+'" type="hidden" value="'+date+'" name="delivery_new['+counter+'][date]" />'+
                    '<input id="textval_'+counter+'" type="hidden" value="'+text+'" name="delivery_new['+counter+'][text]" />'+
                    '<input id="partOfDayval_'+counter+'" class="part-of-day" type="hidden" value="'+partOfDay+'" name="delivery_new['+counter+'][part_of_day]" />'+
                    '<input id="daysval_'+counter+'" type="hidden" value="'+days+'" name="delivery_new['+counter+'][days]" />'+
                    '<input id="phaseval_'+counter+'" type="hidden" value="'+phase+'" name="delivery_new['+counter+'][phase]" />'+
                    '</td></tr>';

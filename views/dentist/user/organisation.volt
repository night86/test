...
{% block content %}
    ...
    {{ form('dentist/user/organisation/' ~ organisation.getId(), 'method': 'post', 'enctype': 'multipart/form-data') }}


    <fieldset class="form-group">

        <legend><a href="{{ url("dentist/user/") }}"><i class="pe-7s-back"></i></a> {{ organisation.getName() }}</legend>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                {% if organisation.logo is not null %}
                    <div class="form-group">
                        <a href="{{ url("dentist/user/deleteorganisationimage/")~organisation.getId() }}" class="btn btn-danger btn-block"><i class="pe-7s-diskette"></i> {{ "Delete image"|t }}</a>
                    </div>
                {% endif %}
            </div>
            {% if organisation.logo is not null %}

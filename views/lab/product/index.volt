...
{% block content %}
    ...
    <div class="toLoader">
        <div class="row">
            <div class="col-md-2">
                <a id="product_contact" class="btn btn-info" style="display: inline-block; margin-left: 15px; margin-top: 20px;">{{ "Can't find the product you want?"|t }}</a>
            </div>
            <div class="col-md-10">
                    ...
                    <div class="col-xs-11">
                        ...
                        <div id="change-list-limit">
                            ...
                            <div class="form-group pull-left">
                                <select id="add-supplier" name="page-limit" class="form-control width-100" style="display: inline-block; margin-left: 100px;">
                                    <option selected="selected">{{ 'Add to shortlist by supplier'|t }}</option>
                                    {% for supplier in filters.getSuppliers() %}
                                        <option value="{{ supplier.id }}">{{ supplier.name }}</option>
                                    {% endfor %}
                                </select>
                                <a id="add-supplier-selected" class="btn btn-primary" style="margin-top: -2px;">
    ...
    {{ partial("modals/confirm", ['id': 'add-shortlist', 'title': 'Add product to favourites'|t, 'content': addShortlistContent, 'additionalClass': 'save-shortlist-data', 'skiptranslation': true]) }}
    {{ partial("modals/addTextarea", ['id': 'product-contact-modal', 'title': 'Can’t find the product you are looking for?'|t, 'content': 'Type all info you have about the product you are looking for: brand, name, article number etc'|t]) }}
    ...
    <script>
        $(function () {
            ...
            $('#product_contact').on('click', function(){
                ...
                $('#product-contact-modal').modal('show');
            });

            $('#confirmAddTextarea').on('click', function(){

                $.ajax({
                    method: 'POST',
                    url: '/lab/product/ajaxcontactproduct/',
                    data: {
                        content: $('#textareaContact').val()
                    },
                ...
                $('#product-contact-modal').modal('hide');
            });

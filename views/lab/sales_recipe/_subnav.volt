            ...
            <ul class="nav navbar-nav">
                {% if currentUser.hasRole('ROLE_LAB_ORGANISATION_EDIT') %}
                    <li {{ controllername is 'user' and actionname is 'organisation' ? 'class="active"' : '' }}>
                        <a href="{{ url('lab/user/organisation/' ~ currentOrganisation.getId()) }}">{{ 'Organisation'|t }}</a>
                    </li>
                {% endif %}
                {% if currentUser.hasRole('ROLE_LAB_USER_INDEX') %}
                    <li {{ controllername is 'user' and actionname is '' ? 'class="active"' : '' }}>
                        <a href="{{ url('lab/user/' ~ currentOrganisation.getId()) }}">{{ 'Users'|t }}</a>
                    </li>
                {% endif %}
                ...
                {% if currentUser.hasRole('ROLE_LAB_SALESRECIPE_INDEX') %}
                    <li {{ controllername is 'sales_recipe' and actionname is ''
                    ? 'class="active"' : '' }}>
                        <a href="{{ url('/lab/sales_recipe/') }}">{{ 'Active Recipes'|t }}</a>
                    </li>
                    <li {{ controllername is 'sales_recipe' and actionname is 'inactive'
                    ? 'class="active"' : '' }}>
                        <a href="{{ url('/lab/sales_recipe/inactive/') }}">{{ 'Recipe archive'|t }}</a>
                    </li>
                {% endif %}

            ...
            <div class="row">
                <div class="col-sm-4">
                    <span class="product-price">
                        {#{% if currentUser.hasRole('ROLE_ADMIN') %}<input type="checkbox" name="marginPriceList" value="{{ productAmount[product['id']]['shortlist'] }}"> {% endif %}#}
                        ...
                        <div class="col-sm-6">
                            <div class="product-actions">
                                ...
                                <a href="{{ url('lab/product/addcart/'~product['id']) }}" id="{{ 'product_'~product['id'] }}" data-id="{{ product['id'] }}"
                                   class="{% if product['special_order'] > 0 %}special-order {% else %} add-cart {% endif %} btn btn-primary"><i
                                ...
                                {% if currentUser.hasRole('ROLE_LAB_SHORTLIST_EDIT') %}
                                    <a href="#" class="btn btn-success edit"
                                       ...
                                       data-shortlist="{{ url('/lab/shortlist/delete/' ~ product['id']) }}"><i
                                                class="pe-7s-edit"></i> {{ "Edit"|t }}</a>
                                {% endif %}
                                <span class="details btn btn-info" data-content="{{ partial("layouts/partial/productDetails", ['product': product]) }}"><i class="pe-7s-help1"></i></span>
                            </div>
                        </div>

    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                {% if currentUser.hasRole('ROLE_GENERAL_INDEX') %}
                    <li {{ controllername is 'general' and actionname is 'organisationEdit' ? 'class="active"' : '' }}>
                        <a href="{{ url('general/organisationEdit/' ~ organisation.getId()) }}">{{ 'Organisation'|t }}</a>
                    </li>
                {% endif %}
                ...
                {#</li>#}
                {#{% endif %}#}
                {% if currentUser.hasRole('ROLE_LAB_INVOICE_INDEX') %}
                    <li {{ controllername is 'invoice'
                    and actionname is ''
                    ? 'class="active"' : '' }}>
                        <a href="{{ url('/lab/invoice/') }}">{{ 'Invoices'|t }}</a>
                    </li>
                {% endif %}
                {#{% if currentUser.hasRole('ROLE_LAB_USER_INDEX') %}
                    <li {{ controllername is 'user' and actionname is '' ? 'class="active"' : '' }}>

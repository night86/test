    ...
    <div class="row">
        <div class="col-lg-12">
            <fieldset class="form-group">
                {{ form('projects/edittask/' ~ projectId ~'/' ~task.getId(), 'method': 'post') }}
                <div class="row">
                    <div class="col-md-12">
                        <legend>{{ "Edit task"|t }}</legend>
                        <div class="col-md-1">
                            <div class="form-group">
                                <label for="status">{{ "Status"|t }}</label>
                                {#{{ check_field('status', 'value':1) }}#}
                                {{ select('status', status, 'required': 'required', 'class': 'form-control', 'value': task.status) }}
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="description">{{ "Description"|t }}</label>
                                {{ text_area('description', 'class': 'form-control', 'value': task.description, 'required': 'required') }}
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="deadline">{{ "Deadline"|t }}</label>
                                {{ text_field('deadline', 'class': 'form-control datepicker-deadline', 'value': task.deadline|dttonl, 'required': 'required') }}
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-gorup">
                                ...
                                <select class="select2-users" name="users[]" required="required">
                                    {% for assigne in task.getAssigneNames() %}
                                        <option value="{{ assigne['id'] }}" selected>{{ assigne['email'] }}</option>
                                    {% endfor %}
                                </select>
                            </div>
                        </div>

        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            ...
            <ul class="nav navbar-nav">
                {% if currentUser.hasRole('ROLE_SIGNADENS_PRODUCT_INDEX') %}
                    ...
                    and (actionname is 'list'
                    or actionname is 'listedit')
                    ? 'class="active"' : '' }}>
                        <a href="{{ url('/signadens/product/list') }}">{{ 'Products'|t }}</a>
                    </li>
                {% endif %}
                {% if currentUser.hasRole('ROLE_SIGNADENS_PRODUCT_INDEX') %}
                    <li {{ controllername is 'product'
                ...
                {% endif %}
                {% if currentUser.hasRole('ROLE_SIGNADENS_INDEX_INDEX') %}
                    <li {{ controllername is 'product' and actionname is 'shopview' ? 'class="active"' : '' }}>
                        <a href="{{ url('/signadens/product/shopview') }}">{{ "Shop view"|t }}</a>
                    </li>
                {% endif %}
                {#{% if currentUser.hasRole('ROLE_SIGNADENS_INDEX_INDEX') %}
                    <li {{ controllername is 'importcode' ? 'class="active"' : '' }}>

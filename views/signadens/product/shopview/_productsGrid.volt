    ...
    <div class="col-md-4 single-item single-item-grid">
        <div class="product-content">
            ...
            <div class="product-numbers database-grid">
                ...
                <p class="product-attribute">{{ "Signa Code"|t }}: {{ product['id'] }}</p>
            ...
            <div class="product-actions">
                ...
                <a href="{{ url('lab/product/addcart/'~product['id']) }}" id="{{ 'product_'~product['id'] }}"
                    data-id="{{ product['id'] }}"
                    class="{% if product['special_order'] > 0 %}special-order {% endif %}add-cart btn btn-primary"
                >
                    <i class="pe-7s-cart"></i> {{ "Add to cart"|t }}
                </a>
                <a href="{{ url('lab/product/addshortlist/'~product['id']) }}" id="{{ 'shortlist_'~product['id'] }}"
                    data-id="{{ product['id'] }}"
                    class="add-shortlist btn btn-primary"
                    data-amount="{% if product['amount_min'] !== null %}{{ product['amount_min'] }}{% else %}1{% endif %}"
                >
                    <i class="pe-7s-cart"></i> {{ "Add to shortlist"|t }}
                </a>

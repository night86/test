...
{% block content %}
    ...
    <div class="row">
        <div class="col-md-4 col-sm-6" style="margin-bottom: 20px;">
            {{ select('map[]', allMaps, 'using': ['id', 'file'], 'useEmpty': true, 'emptyText': 'Select culumns map'|t, 'emptyValue': 0, 'class': 'form-control', 'data-url':url('supplier/import/ajaxmap/'), 'id':'columns-maps') }}
        </div>
    </div>

    <div class="row">
        <div class="col-lg-12">
            {{ 'Important note: all info in mapped columns of the import file will replace the information that is currently in the platform. This also includes images. Do not map the product image column if you will manually add / change images later.'|t }}
        </div>
    </div>
    ...
    <div class="import-steps">
        ...
        <div class="row">
            <div class="col-md-12">
                {{ form('supplier/import/map', 'method': 'post') }}
                <fieldset class="form-group">
                                ...
                                <td>
                                    <select name="map[]" class="form-control" id="map-{{ index }}" required="required">
                                        ...
                                        {% for mapValue in maps %}
                                            <option value="{{ mapValue.id }}" {% if map[index] is defined AND map[index] == mapValue.id %} selected {% endif %}>{{ mapValue.description|t }}{% if mapValue.req is 1 %}*{% endif %}</option>
                                        {% endfor %}
                                    </select>
                                </td>
                            </tr>
                    ...
                    <div class="col-lg-12">
                        <a href="{{ url('supplier/import/index') }}" class="btn btn-primary pull-left"><i class="pe-7s-angle-left"></i> {{ "Previous step"|t }}</a>
